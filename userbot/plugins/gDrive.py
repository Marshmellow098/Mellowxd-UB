...
@mellow.on(admin_cmd(pattern=r"ugdrive ?(.*)"))
async def _(event):
    if event.fwd_from:
        return
...
@mellow.on(admin_cmd(pattern=r"drivesch ?(.*)"))
async def sch(event):
    if event.fwd_from:
        return
    if CLIENT_ID is None or CLIENT_SECRET is None:
...
@mellow.on(admin_cmd(pattern=r"gdrivedir ?(.*)"))
async def _(event):
    if event.fwd_from:
        return
    if CLIENT_ID is None or CLIENT_SECRET is None:
...
@mellow.on(admin_cmd(pattern=r"gfolder ?(.*)"))
async def _(event):
    if event.fwd_from:
        return
