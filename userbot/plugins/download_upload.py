...
@mellow.on(mellow_cmd(pattern="download(?: |$)(.*)", outgoing=True))
async def _(event):
    if event.fwd_from:
        return
...
#@mellow.on(mellow_cmd(pattern="uploadir (.*)", allow_sudo=True))
...
@mellow.on(mellow_cmd(pattern="upload (.*)", outgoing=True))                
async def _(event):
    if event.fwd_from:
        return
    mone = await event.reply("Processing ...")
...
#@mellow.on(mellow_cmd(pattern="uploadas(stream|vn|all) (.*)", allow_sudo=True))
